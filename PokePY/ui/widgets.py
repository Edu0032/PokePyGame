...
def draw_styled_button(
    ...
    bg_color=colors.BATTLE_BUTTON_NORMAL,
    hover_color=colors.BATTLE_BUTTON_HOVER,
    selected=False,
    mouse_pos=None,
    font=None,
):
...
def draw_round_button(
    screen: pygame.Surface, fonts: FontBook, y: int, color, text: str, center_x: int, width: int = 350, height: int = 50
):
    ...
    draw_text(
        screen, fonts, text, rect.centerx, rect.centery, color=colors.BAG_BUTTON_TEXT, font=fonts.large, center=True
    )
    return rect
...
def draw_modal_message(
    screen: pygame.Surface,
    fonts: FontBook,
    message: str,
    bg_color=colors.BATTLE_MENU_BG,
    border_color=colors.BATTLE_MENU_BORDER,
    ...
    rect = pygame.Rect(
        (SCREEN_CONFIG.width - UI_CONFIG.modal_width) // 2,
        (SCREEN_CONFIG.height - UI_CONFIG.modal_height) // 2,
        UI_CONFIG.modal_width,
        UI_CONFIG.modal_height,
    )
