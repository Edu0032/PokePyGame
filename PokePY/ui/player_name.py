...
class PlayerNameView:
    ...
    def draw(self, screen: pygame.Surface, player_name: str, mouse_pos) -> pygame.Rect:
        ...
        draw_text(screen, self.fonts, "PokePy", panel.centerx, panel.y + 52, color=colors.DARK_BLUE, font=self.fonts.xl, center=True)
        draw_text(screen, self.fonts, "Digite seu nome de treinador", panel.centerx, panel.y + 105, color=colors.TEXT_COLOR_DARK, font=self.fonts.large, center=True)
        input_rect = pygame.Rect(panel.x + 65, panel.y + 150, panel.width - 130, 58)
        draw_rounded_box(screen, input_rect, colors.WHITE, radius=10, shadow_color=colors.SHADOW_COLOR_LIGHT, shadow_offset=(2, 2))
        ...
        draw_text(screen, self.fonts, name_text, input_rect.x + 18, input_rect.centery, color=colors.BLACK, font=self.fonts.large, center_y=True)
        draw_text(screen, self.fonts, "ENTER ou botão para iniciar", panel.centerx, input_rect.bottom + 38, color=colors.HUD_TEXT, font=self.fonts.normal, center=True)
        button = pygame.Rect(panel.centerx - 135, panel.bottom - 85, 270, 52)
        draw_styled_button(screen, self.fonts, "Começar Jornada", button, colors.BLACK, colors.CONFIRM_COLOR_OK, colors.SELECTED_COLOR_LIGHT, mouse_pos=mouse_pos, font=self.fonts.large)
        return button
