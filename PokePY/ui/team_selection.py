...
class TeamSelectionView:
    ...
    def draw(self, screen: pygame.Surface, selected_pokemon: list[Pokemon]) -> tuple[list[tuple[pygame.Rect, Pokemon]], pygame.Rect]:
        menu_height = 30 + 60 + 40 + len(self.pokemon_options) * UI_CONFIG.team_button_height + (len(self.pokemon_options) - 1) * UI_CONFIG.team_button_padding + 30 + UI_CONFIG.team_button_height + 30
        ...
        draw_rounded_box(screen, menu_rect, colors.MENU_BOX_COLOR, radius=15, shadow_color=colors.SHADOW_COLOR_DARK, shadow_offset=(5, 5))
        y_cursor = menu_y + 30
        draw_text(screen, self.fonts, "Selecione seu Time", menu_x + UI_CONFIG.team_menu_width // 2, y_cursor + 20, color=colors.TEXT_COLOR_DARK, font=self.fonts.xl, center=True)
        y_cursor += 60
        draw_text(screen, self.fonts, f"Selecionado: {len(selected_pokemon)}/{PLAYER_CONFIG.team_size}", menu_x + UI_CONFIG.team_menu_width // 2, y_cursor, color=colors.TEXT_COLOR_DARK, font=self.fonts.large, center=True)
        y_cursor += 40
        buttons = []
        ...
        for pokemon in self.pokemon_options:
            ...
            draw_text(screen, self.fonts, f"{pokemon.name} ({pokemon.type})", rect.centerx, rect.centery, color=colors.TEXT_COLOR_DARK, font=self.fonts.large, center=True)
        ...
        confirm_color = colors.CONFIRM_COLOR_OK if can_confirm else colors.CONFIRM_COLOR_BAD
        if confirm_rect.collidepoint(mouse_pos):
            confirm_color = tuple(min(value + 20, 255) for value in confirm_color)
        draw_rounded_box(screen, confirm_rect, confirm_color, radius=10, shadow_color=colors.SHADOW_COLOR_DARK, shadow_offset=(2, 2))
        draw_text(screen, self.fonts, "Confirmar Time", confirm_rect.centerx, confirm_rect.centery, color=colors.TEXT_COLOR_LIGHT, font=self.fonts.large, center=True)
