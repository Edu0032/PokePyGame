...
class ExplorationState:
    def update(self, game, current_time_ms: int, current_time_seconds: float) -> None:
        ...
        game.exploration_view.draw_hud(game.screen, game.player, game.current_zone.name, current_time_ms)
        ...
        self._handle_events(game, button_center, button_radius, multiplayer_button)
        if self._try_start_final_boss(game):
            return
        if self._try_random_grass_event(game, moved, current_time_ms, current_time_seconds):
            return
        self._try_change_zone(game)
        game.item_message = game.exploration_view.draw_item_message(game.screen, game.item_message, game.item_message_timer_ms, current_time_ms)
    ...
    def _move_player(self, game) -> bool:
        ...
        if keys[pygame.K_LEFT]:
            ...
            moved = True
        game.player.x = max(PLAYER_CONFIG.boundary_padding, min(game.player.x, SCREEN_CONFIG.width - PLAYER_CONFIG.boundary_padding))
        game.player.y = max(PLAYER_CONFIG.boundary_padding, min(game.player.y, SCREEN_CONFIG.height - PLAYER_CONFIG.boundary_padding))
        return moved

    def _handle_events(self, game, button_center, button_radius: int, multiplayer_button) -> None:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if game.exploration_view.click_hits_backpack(mouse_pos, button_center, button_radius):
    ...
    def _try_start_final_boss(self, game) -> bool:
        if game.current_zone.name != "Zona 3" or game.final_boss_defeated or game.final_boss_triggered:
            return False
        inside_boss_path = SCREEN_CONFIG.height // 2 - WORLD_CONFIG.boss_path_half_height < game.player.y < SCREEN_CONFIG.height // 2 + WORLD_CONFIG.boss_path_half_height
        ...
        game.current_enemy = game.pokemon_factory.create(BOSS_NAME, "Gelo", level=10, hp=400)
        game.state = GameState.BATTLE
        return True

    def _try_random_grass_event(self, game, moved: bool, current_time_ms: int, current_time_seconds: float) -> bool:
        if not moved:
            return False
        if game.current_zone.name == "Zona 3" and game.final_boss_triggered:
            return False
        if not game.map_mask.is_in_grass(game.current_zone.name, game.player.x, game.player.y):
            return False
        ...
        if current_time_seconds - game.last_item_pickup_time >= ITEM_CONFIG.pickup_cooldown_seconds:
            item = game.encounters.random_item(game.current_zone, game.player.x, SCREEN_CONFIG.width, WORLD_CONFIG.boss_trigger_distance_from_right)
            if item:
                self._collect_item(game, item, current_time_ms, current_time_seconds)
        return False

    def _collect_item(self, game, item: str, current_time_ms: int, current_time_seconds: float) -> None:
    ...
    def _try_change_zone(self, game) -> None:
        ...
        if next_index < len(ZONE_DEFINITIONS):
            message = f"Entrando na {ZONE_DEFINITIONS[next_index].name}..."
        elif game.final_boss_defeated:
            message = "Você Venceu o Jogo! Parabéns, Treinador!"
        else:
            message = "Derrote o Boss Final para continuar!"
        game.fade_transition(message)
        if next_index < len(ZONE_DEFINITIONS):
            game.current_zone_index = next_index
            game.player.x = WORLD_CONFIG.zone_entry_x
            game.player.y = SCREEN_CONFIG.height // 2
        elif game.final_boss_defeated:
            draw_text(game.screen, game.fonts, "Você Venceu o Jogo! Parabéns, Treinador!", SCREEN_CONFIG.width // 2, SCREEN_CONFIG.height // 2, color=colors.BLUE, font=game.fonts.xl, center=True)
            pygame.display.flip()
