...
class BattleState:
    def update(self, game, current_time_ms: int, current_time_seconds: float) -> None:
        result = game.battle_controller.run_battle(game.screen, game.player, game.current_battle_pokemon, game.current_enemy, game.current_zone.name)
        if result == BattleResult.WIN:
            ...
            draw_modal_message(game.screen, game.fonts, "Não é possível fugir do Boss!", bg_color=colors.CONFIRM_COLOR_BAD, border_color=colors.RED, text_color=colors.TEXT_COLOR_LIGHT)
            pygame.display.flip()
            pygame.time.wait(1500)
    ...
    def _handle_win(self, game) -> None:
        if game.current_enemy.name == BOSS_NAME:
            ...
            draw_text(game.screen, game.fonts, "O Boss Final foi derrotado!", 200, 300, color=colors.GREEN, font=game.fonts.xl)
            pygame.display.flip()
...
class PokemonSwitchState:
    def update(self, game, current_time_ms: int, current_time_seconds: float) -> None:
        ...
        new_pokemon, switched = game.battle_controller.select_pokemon_in_battle(game.screen, game.player, current, game.current_enemy, game.current_zone.name)
        if switched:
            game.current_battle_pokemon = new_pokemon
            game.battle_view.display_message(game.screen, game.current_battle_pokemon, game.current_enemy, f"Vai, {game.current_battle_pokemon.name}!", game.current_zone.name, 1000)
            game.state = GameState.BATTLE
