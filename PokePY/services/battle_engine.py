...
class BattleEngine:
    ...
    def player_attack(self, attacker: Pokemon, defender: Pokemon, attack_index: int) -> tuple[str, int]:
        if attack_index == 0:
            damage = self.rng.randint(BATTLE_CONFIG.basic_damage_min, BATTLE_CONFIG.basic_damage_max)
            attack_name = "Ataque Básico"
        else:
            damage = self.rng.randint(BATTLE_CONFIG.special_damage_min, BATTLE_CONFIG.special_damage_max)
            attack_name = "Ataque Especial"
        defender.take_damage(damage)
        return attack_name, damage

    def enemy_attack(self, enemy: Pokemon, target: Pokemon) -> int:
        damage = self.rng.randint(BATTLE_CONFIG.enemy_damage_min, BATTLE_CONFIG.enemy_damage_max)
        if enemy.name == BOSS_NAME:
            damage *= BATTLE_CONFIG.boss_damage_multiplier
        target.take_damage(damage)
        return damage

    def reward_victory(self, winner: Pokemon, defeated: Pokemon):
